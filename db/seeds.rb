...
# Create Stages
[
  { :number => 1, :name => "Utrecht > Utrecht", :ridden_on => "2015-07-04", :description => "Tijdrit", :distance => "13.8" },
  ...
  stage = Stage.find_or_initialize_by_number(stage_attributes[:number])
  stage.update_attributes(stage_attributes)
end

puts "Creating or updating default users"

martijn = Player.find_or_initialize_by_name('Martijn van Leeuwen')
...
bastiaan = Player.find_or_initialize_by_name('Bastiaan Langeveld')
